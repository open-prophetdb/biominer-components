...
function ChatFrame(props) {
  const { icon, chatbox, showChatbox, clickIcon, showIcon, iconStyle, children } = props;
  ...
  return (
    <div className="react-chat-frame">
      <div className="react-chat-frame-wrapper">
        ...
        {!showChatbox && <div className="react-chat-frame-custom">{children}</div>}
        {showIcon && (
          <div className="react-chat-frame-icon-container" onClick={clickIcon} style={iconStyle}>
            {icon !== undefined ? icon : <RobotIcon className="react-chat-frame-icon" />}
          </div>
        )}
      </div>
    </div>
