...
const QueryForm: React.FC<AdvancedSearchProps> = (props) => {
  ...
  const [relationTypeOptions, setRelationTypeOptions] = useState<any[]>(
    makeRelationshipTypes(props.edgeStat),
  );
  ...
  useEffect(() => {
    if (relation_types && relation_types.length === 0) {
      if (mode === 'path' || nsteps > 1) {
        setHelpWarning(
          'Use all relation types instead of none, but it may take a long time to search.',
        );
        ...
        setHelpWarning(
          'Please select at least one relation type for performance if you enable prediction.',
        );
        // return;
      } else {
        setHelpWarning('');
    ...
    form
      ...
      .then((values) => {
        ...
        if (props.onOk) {
          let payload = {
            ...values,
            all_relation_types: relationTypeOptions
              ? relationTypeOptions.map((item: any) => item.value)
              : [],
  ...
  return (
    ...
    >
      ...
      <Form.Item
        ...
        rules={[
          { required: isBatchIdsMode ? true : false, message: 'Please enter your expected nodes.' },
        ]}
      >
        <Select
          showSearch
            ...
            <Empty
              description={
                loading
                  ...
                  ? 'Not Found'
                  : `Enter your interested ${node_type} ...`
              }
            />
          }
        ></Select>
      ...
      <Form.Item label="Query Mode" name="query_mode" hidden={!isBatchNodesMode}>
        <Select placeholder="Please select query mode" options={queryModeOptions}></Select>
      </Form.Item>
      <Form.Item
        name="nsteps"
        label="Num of Steps"
      ...
      >
        <Select placeholder="Please select nsteps" options={nStepsOptions}></Select>
      </Form.Item>
      <Form.Item
        name="limit"
