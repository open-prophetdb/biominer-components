...
import type { CanvasStatisticsChartProps } from './index.t';
import type { PlotlyChart } from '../PlotlyViewer/data';
import React, { useEffect, useState } from 'react';
import { genLayout } from './BaseStyle';
import './index.less';

const CanvasStatisticsChart: React.FC<CanvasStatisticsChartProps> = (props) => {
  const { data } = props;
    ...
    <Row className="complex-chart">
      {charts.length === 0 ? (
        ...
        charts.map((chart, index) => {
          return (
            <Col span={12} key={index} style={{ height: '400px' }}>
...
export default CanvasStatisticsChart;
